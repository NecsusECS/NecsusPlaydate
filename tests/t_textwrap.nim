...
suite "Text wrapping":
  ...
  test "Long strings should wrap at breaks":
    check(
      textwrap("foo bar baz and baz and qux and wakka wakka", 80, getWidth).toSeq ==
        @["foo bar", "baz and", "baz and", "qux and", "wakka", "wakka"]
    )

  test "Long words should not be split":
    check(
      textwrap("this_is_a_very_long_word", 80, getWidth).toSeq ==
