import unittest, necsuspd/boxelem, graphics_stub, sequtils

defineBoxElem(Font, Image, Sprite, Color, TextEncoding, DrawMode, BitmapFlip, graphics)

proc `==`*(a: BoxDimens, b: (int, int)): bool =
    return a == (b[0].int32, b[1].int32)

const loremIpsum = "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras molestie tempor odio ut sagittis."
...
proc checkGraphicActions(expected: varargs[string]) =
    ...
    for i in 0..<max(actions.len, expected.len):
        let expect = if i < expected.len: expected[i] else: ""
        let actual = if i < actions.len: actions[i] else: ""
...
suite "Box Elem":
    ...
    test "Changing the draw mode of text":
        text("foo", drawMode = kDrawModeFillWhite).draw(newSprite("testImg", 100, 100), font)
    ...
    test "Right aligned text wrapping":
        text(loremIpsum, wrap = true, align = AlignRight).draw(newSprite("testImg", 300, 100), font)
    ...
    test "Drawing a right aligned image element":
        img(newImage("foo", 10, 20), align = AlignRight).draw(newSprite("testImg", 300, 100), font)
        checkGraphicActions(
            "drawMode(testImg, drawMode: kDrawModeCopy)",
    ...
    test "Drawing a center aligned image element":
        img(newImage("foo", 20, 20), align = AlignCenter).draw(newSprite("testImg", 300, 100), font)
        checkGraphicActions(
            "drawMode(testImg, drawMode: kDrawModeCopy)",
    ...
    test "Setting the max width of a box with text in it":
        maxWidth(200, text(loremIpsum, wrap = true)).draw(newSprite("testImg", 500, 100), font)
        checkGraphicActions(
            "setFont(testImg, font.name: foo)",
            "drawMode(testImg, drawMode: kDrawModeFillBlack)",
            "drawText(testImg, text: Lorem ipsum dolor sit, x: 2, y: 2)",
        ...
        maxWidth(300, text(loremIpsum, wrap = true)).draw(newSprite("testImg", 500, 100), font)
        checkGraphicActions(
            "setFont(testImg, font.name: foo)",
            "drawMode(testImg, drawMode: kDrawModeFillBlack)",
