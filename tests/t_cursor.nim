import
  ...
  necsus,
  necsuspd/[cursor, positioned, fpvec, drawable],
  vmath,
  helpers
...
runTest "Drawable dimensions impact selection":
  let eids = [
    createWithDrawable.with(
      MyCursor(), positioned(0, 0), Selectable(), newDrawable("img", 500, 10)
    ),
    createWithDrawable.with(
      MyCursor(), positioned(0, 300), Selectable(), newDrawable("img", 30, 10)
    ),
    createWithDrawable.with(
      MyCursor(), positioned(250, 300), Selectable(), newDrawable("img", 30, 10)
    ),
  ]
  cursor.init()
  check cursor.selection().to(EntitySelected).entityId == eids[0]
