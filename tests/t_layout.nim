...
template createLayouter(entities: untyped) =
    proc lookup(eid: EntityId): Option[(ptr Positioned, TestSprite)] {.gcsafe, raises: [].} =
        {.cast(gcsafe).}:
            let i = eid.uint.int
            if i < entities.len:
...
suite "Layout Elem":
    ...
    test "Stacked entities should be layed out vertically":
        ...
        let layout = stackLayout(EntityId(0).spriteLayout, EntityId(1).spriteLayout, EntityId(2).spriteLayout)
        let dimens = layout.layout(layouter, x = 5, y = 7, width = 100)
    ...
    test "Row entities should be layed out horizontally":
        ...
        let layout = rowLayout(EntityId(0).spriteLayout, EntityId(1).spriteLayout, EntityId(2).spriteLayout)
    ...
    test "Right aligned entity inside of center columns":
        var entities = [
            (positioned(0, 0), (10, 20)),
            (positioned(0, 0), (30, 40)),
        ]
        createLayouter(entities)

        let layout = horizLayout(AlignCenter,
            rowLayout(
                EntityId(0).spriteLayout,
                EntityId(1).spriteLayout(AlignRight),
            )
        )
