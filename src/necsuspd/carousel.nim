import necsus, positioned, vmath, easing, inputs, findDir, util, time, vec_tools, playdate/api, options
...
type
    ...
    CarouselChanged*[T] = object
        ## Event sent when a carousel gets a new value
...
proc reset*[T](bundle: Bundle[Carousel[T]], target: IVec2, easing: EasingCalc[Vec2], duration: float32) =
    ...
    for (eid, startPos) in bundle.firstElement:
        ...
        bundle.data := CarouselData[T](
            animating: false, target: target, selected: eid, easing: easing, duration: duration)
...
proc checkNewSelection[T](bundle: Bundle[Carousel[T]]) =
    ...
    for data in bundle.data:
        for selected in bundle.determineSelection():
            let origin = bundle.findElem(selected).mapIt(it[1][]).orElse(positioned(data.target))

            for button in bundle.buttons:
                for (newPosition, newSelection) in findDir[EntityId](eligibleCards(bundle), button, origin):
                    bundle.data := CarouselData[T](
                        animating: true,
                        target: data.target,
