...
type
    TreeSeqKind = enum Empty, Single, Double, Repeat
...
proc len*(tree: TreeSeq): int32 =
    ...
    of Empty: return 0
    of Single: return 1
    of Double: return tree.sizeCache
    of Repeat: return tree.sub.len * tree.count

proc `[]`*[T](tree: TreeSeq[T], index: SomeInteger): T =
    when compileOption("boundChecks"):
        if unlikely(index < 0 or index >= tree.len):
            raise newException(IndexDefect, fmt"TreeSeq index {index} is out of bounds (length is: {tree.len})")

    case tree.kind
    of Empty: discard
    of Single: return tree.value
    of Double:
        if index < tree.a.len:
            return tree.a[index]
        else:
...
proc treeSeq*(a: auto, b: auto): auto =
    let subA = a.treeSeq
    let subB = b.treeSeq
    return TreeSeq[type(subA[0])](kind: Double, a: subA, b: subB, sizeCache: subA.len + subB.len)
