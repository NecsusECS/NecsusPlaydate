...
template defineBoxElem*[T](
    LCDFont, LCDBitmap, LCDSprite, LCDSolidColor, PDStringEncoding, LCDBitmapDrawMode,
      LCDBitmapFlip: typedesc,
    pdGraphics: T,
) =
  ...
  proc `$`*(box: BoxElem): string =
    ...
    of MaxWidthElem:
      result.add($box.maxWidth & " for " & $box.maxWidthInner)
    result.add(")")
  ...
  proc recursiveDraw(
    ...
    of RowElem:
      ...
      for nested in box.row:
        let nestedArea: DrawArea =
          (left: result.width, right: area.right, top: area.top)
    ...
    of MaxWidthElem:
      ...
      result =
        recursiveDraw(executeDraw, box.maxWidthInner, img, defaultFont, nestedArea)
...
when not defined(unittests):
  ...
  defineBoxElem(
    LCDFont, LCDBitmap, LCDSprite, LCDSolidColor, PDStringEncoding, LCDBitmapDrawMode,
    LCDBitmapFlip, playdate.graphics,
  )
