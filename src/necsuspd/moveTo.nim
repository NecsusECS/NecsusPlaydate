...
type
  MoveTo*[Filter] = object ## Bundle needed to attach a movement
  ...
  CallbackMoveTo*[T] = object ## Creates callback based movers
    spawn: Spawn[(MoveCallback, T)]
    time: GameTime

proc defaultOnComplete() =
  discard
...
template calculate(
    originValue, targetValue: typed,
    startTime, duration, currentTime: float32,
    exec: untyped,
): untyped =
...
proc calculate*(
    movement: MovingTo | ptr MovingTo, currentTime: float32
): tuple[newPos: IVec2, isDone: bool] =
  ## Calculates the position for the given movement definition
  let (newPos, isDone) = calculate(
    movement.origin, movement.target, movement.startTime, movement.duration, currentTime
  ):
    movement.origin +
      vec2(
