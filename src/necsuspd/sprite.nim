...
proc offsetFix(sprite: LCDSprite, anchor: Anchor): IVec2 =
    ...
    case anchor.lock
    of AnchorTopLeft, AnchorBottomLeft: result.x += sprite.getImage.getSize.width.int32 div 2
    of AnchorTopMiddle, AnchorMiddle, AnchorBottomMiddle: discard
    of AnchorTopRight, AnchorBottomRight: result.x -= sprite.getImage.getSize.width.int32 div 2

    case anchor.lock
    of AnchorTopLeft, AnchorTopMiddle, AnchorTopRight: result.y += sprite.getImage.getSize.height.int32 div 2
    of AnchorMiddle: discard
    of AnchorBottomLeft, AnchorBottomMiddle, AnchorBottomRight: result.y -= sprite.getImage.getSize.height.int32 div 2
...
proc width*(sprite: Sprite | Animation): auto = sprite.sprite.getImage.getSize.width

proc height*(sprite: Sprite | Animation): auto = sprite.sprite.getImage.getSize.height

proc def*[S](animation: Animation[S]): AnimationDef[S] = animation.def
...
proc animation*[S : enum](
    ...
    assert(frames.len > 0)
    AnimationDef[S](sheet: sheet, frames: frames.toSeq, anchor: anchor.toAnchor, loop: loop)
...
proc newBlankSprite*(
    ...
    return newBitmapSprite(playdate.graphics.newBitmap(width, height, color), zIndex, anchor, absolutePos)
...
proc `[]`[S](anim: ptr Animation[S], frame: int32): LCDBitmap =
...
proc newSheet*[S](
    ...
    result.sprite.setImage((addr result)[result.frame], kBitmapUnflipped)
...
proc buidSpriteMover*[S](): auto =
    ...
    return proc (
        ...
        animated: Query[(ptr Animation[S], Positioned)],
...
proc buildSpriteAdvancer*[S](): auto =
    ...
    return proc(
        time: GameTime,
        elements: FullQuery[(ptr Animation[S], Option[Unpausable])],
        events: Outbox[Keyframe[S]]
    ) =
        let now = time.get.float32
        for eid, (anim, unpausable) in elements:
            if anim.sprite.visible and (not anim.paused or unpausable.isSome) and anim.nextFrameTime <= now:
...
proc setBitmapMask*(
    sprite: Sprite,
    img: LCDBitmap = playdate.graphics.newBitmap(sprite.width, sprite.height, kColorWhite)
) =
    discard sprite.getImage.setBitmapMask(img)

proc visible*(sprite: Sprite | Animation): bool {.inline.} = sprite.sprite.visible

proc `visible=`*(sprite: Sprite | Animation, visible: bool) {.inline.} = sprite.sprite.visible = visible

proc `zIndex`*(sprite: Sprite | Animation): int16 {.inline.} = sprite.sprite.zIndex

proc `zIndex=`*(sprite: Sprite | Animation, index: auto) {.inline.} = sprite.sprite.zIndex = index.int16
...
proc resetPooledValue*(bitmap: LCDBitmap) = discard

proc restorePooledValue*(bitmap: LCDBitmap) = discard

proc `$`*(sprite: Sprite): string =
    {.cast(gcsafe).}:
        let isVisible = if sprite.sprite.visible: "visible" else: "hidden"
        return fmt"Sprite({sprite.sprite.bounds}, {isVisible}, zIndex={sprite.sprite.zIndex})"
