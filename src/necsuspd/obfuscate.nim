...
type
    Keypad*[A, B: static int] = object
        key1*: array[A, byte]
        key2*: array[B, byte]

proc apply(byt: byte, i: int32, random: var MersenneTwister, keypad1, keypad2: openarray[byte]): byte =
    when defined(noObfuscation):
        result = byt
    else:
        result = byt xor keypad1[i mod keypad1.len]  xor keypad2[i mod keypad2.len] xor random.randomInt(byte)
