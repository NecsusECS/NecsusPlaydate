...
type Percent* = distinct int32 ## Integer math based percentages
...
proc toFloat*(pct: Percent): float32 =
  int32(pct).toBiggestFloat / RESOLUTION.float32

proc `$`*(pct: Percent): string =
  fmt"{(pct.toFloat * 100):.2f}%"
...
template `*`*[T](versus: Percent, value: T): T =
  value * versus
