...
type
    TriggerKind = enum TriggerBoxKind, TriggerJoinKind
...
proc `$`*(box: AnyTriggerBox): string =
    case box.kind
    of TriggerBoxKind: return fmt"Box(dimens: {box.dimens}, offset: {box.offset})"
    of TriggerJoinKind: return fmt"Join{box.a}, {box.b})"
...
proc height*(bounds: BoxBounds): int32 = bounds.maxY - bounds.minY

proc height*(trigger: AnyTriggerBox): int32 = trigger.bounds.height

proc width*(bounds: BoxBounds): int32 = bounds.maxX - bounds.minX

proc width*(trigger: AnyTriggerBox): int32 = trigger.bounds.width

proc offset*(bounds: BoxBounds): IVec2 = ivec2(bounds.minX, bounds.minY)

proc offset*(trigger: AnyTriggerBox): IVec2 = trigger.bounds.offset

proc dimens*(bounds: BoxBounds): IVec2 = ivec2(bounds.width, bounds.height)
...
proc triggerBox*(width, height: SomeInteger, zIndex: enum, offset: IVec2 = ivec2(0, 0)): TriggerBox =
    result = TriggerBox(kind: TriggerBoxKind, dimens: ivec2(width.int32, height.int32), offset: offset)
    when enableDebugSprite:
        result.debugSprite = newBlankSprite(width, height, zIndex, AnchorTopLeft, kColorClear)
...
proc overlaps[A : AnyTriggerBox, B: AnyTriggerBox](aBox: A; bBox: B; aPos, bPos: Positioned): bool =
    ## Whether two trigger boxes overlap
    case aBox.kind
    of TriggerBoxKind:
        case bBox.kind
        of TriggerBoxKind: return overlaps(rect(aBox, aPos), rect(bBox, bPos))
        of TriggerJoinKind: return overlaps(aBox, bBox.a, aPos, bPos) or overlaps(aBox, bBox.b, aPos, bPos)
    of TriggerJoinKind: return overlaps(aBox.a, bBox, aPos, bPos) or overlaps(aBox.b, bBox, aPos, bPos)
...
proc overlaps[A: AnyTriggerBox, B](aBox: A; aPos: Positioned, b: B): bool =
    ...
    when B is AnyOverlapBox:
        ...
        case aBox.kind
        of TriggerBoxKind: return overlaps(b, rect(aBox, aPos))
        of TriggerJoinKind: return overlaps(aBox.a, aPos, b) or overlaps(aBox.b, aPos, b)
