...
proc address(value: ref): string = "0x" & cast[uint64](addr value[]).toHex
...
proc prepareApis(def, objType: NimNode): tuple[builderImpl, callBuilder, poolingProc: NimNode] =
...
macro pooled*(size: int32, def: typed): untyped =
    ...
    poolingProc.body = readFromPool(def.name.strVal.newLit, storage, objType, size, callBuilder)
...
macro multiPooled*(size: int32, def: typed): untyped =
    ...
    poolingProc.body = readFromPool(
        readName,
        nnkBracketExpr.newTree(storage, keySym),
        objType,
        size,
        callBuilder
    )
