...
proc getState(control: Bundle[CrankData]): var CrankState =
  control.state.getOrPut:
    CrankState(lastUpdateTick: control.currentTickId())
...
proc recalculateRotations(
    control: Bundle[CrankData], state: var CrankState = control.getState
): float32 =
