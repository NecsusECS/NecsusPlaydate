...
proc getPixel(rect: GrayRectangle, x, y: int): int = rect.shade
...
proc drawFade*(img: LCDBitmap, opacity: int) =
    ...
    GrayRectangle(shade: opacity.clamp(0, 255), width: data.width, height: data.height)
        .orderedDither(data, BlackAndWhiteIntPalette, Bayer4x4)

proc createFadePrecalculation*(baseMask: LCDBitmap, size: int, minOpacity: int = 0, maxOpacity: int = 255): PrecalculatedFade =
...
proc draw*(fade: PrecalculatedFade, sprite: Sprite, existingValue: Val[FadeState], t: float32) =
