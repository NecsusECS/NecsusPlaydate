...
template buildStep[T](
    index: StepId, steps: MultiStep[T], condition: bool, action: untyped
) =
...
template awaitButton*[T](
    steps: MultiStep[T],
    triggers: set[PDButton],
    debounce: float32 = 0.2,
    condition: bool = true,
) =
  const index = stepId(T)
  buildStep(index, steps, condition):
    for event in steps.buttons:
      if event in triggers and
          steps.time() - steps.state.getOrPut().lastExecuteTime >= debounce:
