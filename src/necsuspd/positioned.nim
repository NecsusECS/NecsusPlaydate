...
proc toFPVec2*(pos: Positioned | ptr Positioned): FPVec2 = pos.pos + pos.offset

proc toIVec2*(pos: Positioned | ptr Positioned): IVec2 = pos.toFPVec2().toIVec2()

proc toVec2*(pos: Positioned | ptr Positioned): Vec2 = pos.toFPVec2().toVec2()

proc x*(pos: Positioned | ptr Positioned): auto = pos.x

proc y*(pos: Positioned | ptr Positioned): auto = pos.y

template assign(name) =
    proc `name =`*(pos: ptr Positioned, value: int32) = pos.pos.`name` = value.fp(FPVecPrecision)
    proc `name =`*(pos: ptr Positioned, value: int) = pos.pos.`name` = value.fp(FPVecPrecision)
...
proc angle*(pos: Positioned): FPInt32 = (pos.lastPos - pos.pos).angle.toDegrees * -1
